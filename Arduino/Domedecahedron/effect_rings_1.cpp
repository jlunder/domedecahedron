...
void * effect_rings_1_initialize(void);
void effect_rings_1_process(void * state, fix16_t delta_time,
...
typedef struct {
    ...
    color_t buf[DDH_TOTAL_VERTICES];
} effect_rings_1_state_t;

effect_t const effect_rings_1 = {
    &effect_rings_1_initialize,
    &free,
    &effect_rings_1_process,
    NULL,
    NULL,
};
...
void * effect_rings_1_initialize(void)
{
    effect_rings_1_state_t * state =
        (effect_rings_1_state_t *)malloc(sizeof (effect_rings_1_state_t));
    
    memset(state, 0, sizeof (effect_rings_1_state_t));
    
    return state;
}

void effect_rings_1_process(void * voidp_state, fix16_t delta_time,
    color_t buf[DDH_TOTAL_VERTICES])
{
    ...
    */
    ...
    if(in0 == 0 && in1 == 0 && in2 == 0) {
        ...
        if(state->no_interaction_time > fix16_from_int(10)) {
            ...
            in2 = (eu_random() % fix16_from_int(1)) < (uint32_t)delta_time ?

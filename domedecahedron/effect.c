...
void effect_process_plasma0(color_t buf[DDH_TOTAL_VERTICES])
{
    ...
    color_t ca = color_make(
        ...
        fix16_mul(fix16_sin(fix16_mul(t / 70, 2 * fix16_pi)) + fix16_from_int(1), 127));
    color_t cb = color_make(
        ...
        fix16_mul(fix16_sin(fix16_mul(t / 70, 2 * fix16_pi) + fix16_from_int(3)) + fix16_from_int(1), 127));
    int32_t l;
    ...
    for(size_t i = 0; i < DDH_TOTAL_VERTICES; ++i) {
        ...
        fix16_t plasma_b =
            eu_sin_dist3(
                vector3_make(
                    ...
                    3 * fix16_cos(fix16_mul(t / 9, 2 * fix16_pi) + fix16_from_int(3)),
                    fix16_from_int(-2)),
                ddh_vertex_coords_fix[i],
                fix16_pi * 13 / 7,
                0) + fix16_from_int(1);
        ...
        fix16_t plasma_c =
            eu_sin_dist3(
                vector3_make(
                    4 * fix16_sin(fix16_mul(t / 25, 2 * fix16_pi) + fix16_from_int(4)) + fix16_from_int(2),
                    6 * fix16_cos(fix16_mul(t / 25, 2 * fix16_pi) + fix16_from_int(7)) + fix16_from_int(2),
                    fix16_from_int(-2)),
                ddh_vertex_coords_fix[i],
                fix16_pi * 3 / 5,
                0) + fix16_from_int(1);
